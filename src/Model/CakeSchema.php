...
class CakeSchema extends CakeObject
{
    ...
    protected function _columns(&$model)
    {
        ...
        foreach ($fields as $name => $value) {
            if ($model->primaryKey === $name && !$hasPrimaryAlready && !isset($value['key'])) {
                $value['key'] = 'primary';
            }
            if (!str_starts_with($value['type'], 'enum')) {
                if (!isset($db->columns[$value['type']])) {
                    trigger_error(__d('cake_dev', 'Schema generation error: invalid column type %s for %s.%s does not exist in DBO', $value['type'], $model->name, $name), E_USER_NOTICE);
                    continue;
                } else {
