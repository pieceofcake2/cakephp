...
class Model extends CakeObject implements CakeEventListener
{
     ...
     */
    public string $useDbConfig = 'default';
     ...
     */
    public ?BehaviorCollection $Behaviors = null;
     ...
     * @var string|null
     */
    public ?string $findQueryType = null;
...
// @codingStandardsIgnoreStart
     ...
     * @var bool
     */
    public bool $useConsistentAfterFind = true;
     ...
     * @var string|int|null
     */
    protected string|int|null $_insertID = null;
     ...
     */
    protected bool $_sourceConfigured = false;
    ...
    public function __construct(
        ...
        $this->_mergeVars(['findMethods'], Model::class);
        ...
        foreach ($this->_associations as $type) {
            ...
            if (!empty($association)) {
                foreach ($association as $assoc => $value) {
                    ...
                    if (!isset($value['className']) && str_contains($assoc, '.')) {
                        unset($association[$assoc]);
    ...
    public function schema(string|bool $field = false): ?array
    {
        ...
        return $this->_schema[$field] ?? null;
    }
    ...
    public function getColumnType(?string $column): ?string
    {
        ...
        $startQuote = $db->startQuote ?? null;
        $endQuote = $db->endQuote ?? null;
     ...
     * @return array|bool See Model::save() False on failure or an array of model data on success.
    ...
    protected function _doSave(?array $data = null, array $options = []): array|bool
    {
        ...
        if ($options['callbacks'] === true || $options['callbacks'] === 'before') {
            ...
            [$event->break, $event->breakOn] = [true, [false, null]];
            ...
            if (!empty($this->id)) {
                ...
                if (empty($this->data[$this->alias][$this->primaryKey]) && $this->_isUUIDField($this->primaryKey)) {
                    if (array_key_exists($this->primaryKey, $this->data[$this->alias])) {
                        ...
                        [$fields[], $values[]] = [$this->primaryKey, CakeText::uuid()];
                    }
                }
    ...
    public function buildQuery(string $type = 'first', array $query = []): ?array
    {
        ...
        if ($query['callbacks'] === true || $query['callbacks'] === 'before') {
            ...
            [$event->break, $event->breakOn, $event->modParams] = [true, [false, null], 0];
            $this->getEventManager()->dispatch($event);
        ...
        if ($state === 'before') {
            if (empty($query['fields'])) {
                ...
                if (count($query['fields']) === 1) {
                    ...
                    for ($i = 0; $i < 3; $i++) {
                        if (!str_contains($query['fields'][$i], '.')) {
    ...
    public function setDataSource(?string $dataSource = null): void
    {
        ...
        $defaultProperties = get_class_vars(static::class);
     ...
     *
     * @param string|null $type If null this deletes cached views if Cache.check is true
     *     Will be used to allow deleting query cache also
     * @return bool True on delete, false otherwise
     */
    protected function _clearCache(?string $type = null): bool
    {
     ...
     *
     * @param ModelValidator|null $instance Model validator instance.
     *  If null a new ModelValidator instance will be made using current model object
     * @return ModelValidator
     */
    public function validator(?ModelValidator $instance = null): ModelValidator
    {
        if ($instance) {
