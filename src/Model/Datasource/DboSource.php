...
class DboSource extends DataSource
{
    ...
    public function cacheMethod(string $method, string $key, mixed $value = null): mixed
    {
        ...
        if ($value === null) {
            return static::$methodCache[$method][$key] ?? null;
        }
    ...
    public function create(
        ...
        if ($this->execute($this->renderStatement('create', $query))) {
            if (empty($id)) {
                $id = $this->lastInsertId($this->fullTableName($model, false, false));
            }
    ...
    public function queryAssociation(
        ...
        if (!is_array($resultSet)) {
            throw new CakeException(__d('cake_dev', 'Error in Model %s', $model::class));
        }
    ...
    public function prepareFields(
        ...
        if (empty($queryData['fields'])) {
            ...
            if (
                ...
                (!str_contains($passedFields[0], $assocFields[0]) && !preg_match('/^[a-z]+\(/i', $passedFields[0]))
            ) {
                ...
                if ($external) {
                    ...
                    if ($model->name !== $LinkModel->name) {
                        ...
                        foreach ($conditions as $key => $condition) {
                            if (is_numeric($key) && str_contains($condition, $modelAlias . '.')) {
    ...
    public function buildJoinStatement(array $join): string
    {
        ...
        if (!empty($data['table']) && (!is_string($data['table']) || !str_starts_with($data['table'], '('))) {
    ...
    protected function _prepareUpdateFields(
        ...
        foreach ($fields as $field => $value) {
            ...
            } elseif (!$alias && str_contains($field, '.')) {
            ...
            if ($quoteValues) {
                $update .= $this->value($value, $model->getColumnType($field), $schema[$field]['null'] ?? true);
     ...
     *
     * @param array<string, mixed> $conditions Array or string of conditions
     * @param bool $quoteValues If true, values should be quoted
     * @param Model|null $model A reference to the Model instance making the query
    ...
    protected function _parseKey(
        ...
        if (!str_contains($key, ' ')) {
            ...
            if (!preg_match($operatorMatch, trim($operator)) && str_contains($operator, ' ')) {
                $key = $key . ' ' . $operator;
                $split = strrpos($key, ' ');
    ...
    public function order(
        ...
        while (!empty($keys)) {
            ...
            if (is_string($key) && str_contains($key, ',') && !preg_match('/\(.+\,.+\)/', $key)) {
    ...
    public function length(string $real): string|int|null
    {
        ...
        if ($isFloat && str_contains($length, ',')) {
            return $length;
        }
    ...
    public function buildColumn(array $column): ?string
    {
        ...
        if (!isset($this->columns[$type]) && !str_starts_with($type, 'enum')) {
