...
class Dispatcher implements CakeEventListener
{
    ...
    protected function _attachFilters(CakeEventManager $manager): void
    {
        ...
        foreach ($filters as $index => $filter) {
            $settings = [];
            if (is_array($filter) && !is_int($index) && !isset($filter['callable'])) {
                $settings = $filter;
                $filter = $index;
            }
            if (is_string($filter)) {
            ...
            if (is_string($filter['callable'] ?? null)) {
                ...
                $manager->attach($filter['callable'] ?? null, 'Dispatcher.' . $on . 'Dispatch', $options);
            }
        }
    }
     ...
     * @return class-string<Controller>|false Name of controller class name
     */
