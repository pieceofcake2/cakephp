...
class Router
{
    ...
    protected static function _validateRouteClass(string $routeClass): string
    {
        if (
            ...
            (!class_exists($routeClass) || !is_subclass_of($routeClass, CakeRoute::class))
        ) {
     ...
     *
     * @param array|string $controller A controller name or array of controller names (i.e. "Posts" or "ListItems")
    ...
    public static function getRequest($current = false)
    {
        if ($current) {
            ...
            return static::$_requests[$i] ?? null;
        }
    ...
    public static function url(array|string|null $url = null, array|bool $full = false): string
    {
        ...
        if (empty($url)) {
            ...
            foreach (static::$_prefixes as $prefix) {
                ...
                if (isset($url[$prefix]) && str_starts_with($url['action'], $prefix . '_')) {
    ...
    protected static function _handleNoRoute($url)
    {
        ...
        foreach (static::$_prefixes as $prefix) {
            $prefixed = $prefix . '_';
            if (!empty($url[$prefix]) && str_starts_with($url['action'], $prefixed)) {
     ...
     *
     * @param array|string $q Query string Either a string of already compiled query string arguments or
    ...
    {
        $params = Router::reverseToArray($params);
