...
class ClassRegistry
{
    ...
    public static function init($class, $strict = false)
    {
        ...
        foreach ($objects as $settings) {
            ...
            if (is_array($settings)) {
                ...
                $model = $_this->_duplicate($alias, $class, $actualClass);
                if ($model) {
                ...
                if (class_exists($actualClass) || interface_exists($actualClass)) {
                    $reflection = new ReflectionClass($actualClass);
                ...
                if (!isset($instance)) {
                    $appModel = App::className('AppModel', 'Model');
                    if (!$appModel) {
                        $appModel = 'AppModel';
                    }
                    if ($strict) {
     ...
     */
    public static function getObject(string $key): object|false
    {
        $_this = ClassRegistry::getInstance();
        $key = Inflector::underscore($key);
        $return = false;
    ...
    protected function &_duplicate($alias, $class, $actualClass = null)
    {
        ...
        if (static::isKeySet($alias)) {
            ...
            if (is_object($model) && (($actualClass && $model instanceof $actualClass) || $model instanceof $class || $model->alias === $class)) {
                $duplicate = $model;
            }
            unset($model);
        }
