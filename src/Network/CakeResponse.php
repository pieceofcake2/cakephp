...
class CakeResponse
{
    ...
    protected function _setCookies(): void
    {
        foreach ($this->_cookies as $name => $c) {
            setcookie(
                $name,
                $c['value'],
                ['expires' => $c['expire'], 'path' => $c['path'], 'domain' => $c['domain'], 'secure' => $c['secure'], 'httponly' => $c['httpOnly']],
            );
        }
    }
     ...
     *
     * @param CakeRequest $request Request object
     * @param array|string $allowedDomains List of allowed domains, see method description for more details
     * @param array|string $allowedMethods List of HTTP verbs allowed
