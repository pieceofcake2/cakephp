...
class Controller extends CakeObject implements CakeEventListener
{
    ...
    public function __construct(?CakeRequest $request = null, ?CakeResponse $response = null)
    {
        ...
        if ($this->name === null) {
            ...
            // Remove "Controller" suffix (10 characters)
    ...
    public function __isset(string $name): bool
    {
        ...
        if (is_array($this->uses)) {
            foreach ($this->uses as $modelClass) {
                [, $class] = pluginSplit($modelClass, true);
                if ($name === $class) {
    ...
    public function setRequest(CakeRequest $request): void
    {
        ...
        $this->view = $request->params['action'] ?? null;
    ...
    protected function _mergeControllerVars(): void
    {
        ...
        if (!empty($this->plugin)) {
            $pluginController = App::className($this->plugin . '.' . $this->plugin . 'AppController', 'Controller');
            if (!$pluginController) {
     ...
     *
     * @param array<int, string>|int|null $code If $code is an integer, then the corresponding code/message is
     ...
     * @return array<int, string>|bool|null Associative array of the HTTP codes as keys, and the message
    ...
    public function redirect(
        ...
        [$event->break, $event->breakOn, $event->collectReturn] = [true, false, true];
     ...
     *       status: int|null,
     *       exit: bool
     *  }|array|string|null> $response Response from beforeRedirect callback
    ...
    public function render(string|bool|null $view = null, ?string $layout = null): ?CakeResponse
    {
        ...
        foreach ($models as $currentModel) {
            ...
            if ($currentObject instanceof Model) {
                if (!isset($this->request->params['models'][$currentObject->alias])) {
                    $fullClassName = $currentObject::class;
                    $plugin = null;
                    if (str_contains($fullClassName, '\\')) {
                        $parts = explode('\\', $fullClassName);
                        $className = array_pop($parts);

                        if (count($parts) > 0 && !in_array($parts[0], ['Cake', 'App'])) {
                            $plugin = $parts[0];
                        }
                    } else {
                        $className = $currentObject::class;
