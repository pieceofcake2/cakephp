...
class Shell extends CakeObject
{
    ...
    public function __construct(
        ...
        if (!$this->name) {
            ...
            $this->name = Inflector::camelize(str_replace(['Shell', 'Task'], '', $className));
        }
    ...
    public function hasMethod(string $name): bool
    {
        try {
            ...
            if (!$method->isPublic() || str_starts_with($name, '_')) {
                return false;
            }
     ...
     * @return void
     */
    protected function _useLogger(bool $enable = true): void
    {
        if (!$enable) {
            CakeLog::drop('stdout');
            CakeLog::drop('stderr');
