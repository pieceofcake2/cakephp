 ...
 * @deprecated This class will be removed in 3.0. You should use a separate response cache like Varnish instead.
...
class CacheHelper extends AppHelper
{
    ...
    protected function _writeFile($content, $timestamp, $useCallbacks = false)
    {
        ...
        // Check if the controller class is namespaced
        if (strpos($controllerClass, '\\') !== false) {
            // Namespaced controller - no need for App::uses()
            $appUsesCode = '';
        } else {
            // Legacy non-namespaced controller - use App::uses()
            if (empty($this->_View->plugin)) {
                $appUsesCode = sprintf(
                    <<<'TEMPLATE'
                    App::uses('%s', 'Controller');
                    TEMPLATE,
                    $controllerClass,
                );
            } else {
                $appUsesCode = sprintf(
                    <<<'TEMPLATE'
                    App::uses('%sAppController', '%s.Controller');
                    App::uses('%s', '%s.Controller');
                    TEMPLATE,
                    $this->_View->plugin,
                    $this->_View->plugin,
                    $controllerClass,
        ...
        $requestSerialized = base64_encode(serialize($this->request));
        $responseType = $this->_View->response->type();
        $plugin = $this->_View->plugin;
        $helpersSerialized = base64_encode(serialize($this->_View->helpers));
        $layout = $this->_View->layout;
        $theme = $this->_View->theme;
        $viewVarsSerialized = base64_encode(serialize($this->_View->viewVars));

        $callbacksCode = $useCallbacks ? sprintf(<<<'CALLBACKS'
        ...
        $file .= sprintf(
            <<<'TEMPLATE'
            %s
            $request = unserialize(base64_decode('%s'));
            $response->type('%s');
            $controller = new %s($request, $response);
            $controller->plugin = $this->plugin = '%s';
            $controller->helpers = $this->helpers = unserialize(base64_decode('%s'));
            $controller->layout = $this->layout = '%s';
            $controller->theme = $this->theme = '%s';
            $controller->viewVars = unserialize(base64_decode('%s'));
