...
class XmlTest extends CakeTestCase
{
    ...
    public function testBuild()
    {
        ...
        $xml = CORE_TESTS . DS . 'Fixture' . DS . 'sample.xml';
        $obj = Xml::build($xml);
...
XML;
    ...
    {
        $rss = file_get_contents(CORE_TESTS . DS . 'Fixture' . DS . 'rss.xml');
...
XML;
    ...
    {
        $xmlRequest = Xml::build(CORE_TESTS . DS . 'Fixture' . DS . 'soap_request.xml');
        $expected = [
            'Envelope' => [
        ...
        $xmlResponse = Xml::build(CORE_TESTS . DS . 'Fixture' . DS . 'soap_response.xml');
        $expected = [
            'Envelope' => [
                '@soap:encodingStyle' => 'http://www.w3.org/2001/12/soap-encoding',
                'soap:Body' => [
