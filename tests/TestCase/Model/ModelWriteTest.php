...
class TestAuthor extends Author
{
    ...
    protected ?DataSource $_dataSourceObject = null;

    public array $dataForAfterSave = [];
...
class ModelWriteTest extends BaseModelTest
{
    ...
    public function testSaveAllDeepAssociated()
    {
        ...
        unset(
            ...
            $result['User']['created'],
            $result['User']['updated'],
            $result['Comment'][0]['created'],
            $result['Comment'][0]['updated'],
        );
        $this->assertEquals($expected, $result);
    }
    ...
    public function testSaveAllValidation()
    {
        ...
        $this->assertEquals(static::date(), $result[3]['Post']['created']);
        unset(
            $result[0]['Post']['updated'],
            $result[1]['Post']['updated'],
            $result[3]['Post']['updated'],
            $result[3]['Post']['created'],
        );
        $this->assertEquals($expected, $result);
        $this->assertEquals($errors, $TestModel->validationErrors);
    ...
    public function testSaveAssociated()
    {
        ...
        unset(
            $result[3]['Post']['updated'],
            $result[3]['Post']['created'],
            $result[3]['Author']['updated'],
            $result[3]['Author']['created'],
        );
        $this->assertEquals($expected, $result[3]);
        $this->assertEquals(4, count($result));
    ...
    public function deleteMe($event): void
    {
        $model = $event->subject;
        $model->getDataSource()
            ->delete($model, [$model->alias . '.' . $model->primaryKey => $model->id]);
    }
    ...
    public function testTransactionOnNestedSaveMany()
    {
        $this->loadFixtures('Post');
        $post = new TestPost();
        $post->getEventManager()
            ->attach([$this, 'nestedSaveMany'], 'Model.afterSave');
    ...
    public function testTransactionOnNestedSaveAssociated()
    {
        ...
        $db = $this->_getMockDboSource(['begin', 'commit', 'rollback']);
        $db->expects($this->exactly(2))
            ->method('begin')->will($this->returnValue(true));
        $db->expects($this->never())
            ->method('commit')
            ->willReturn(true);
        $db->expects($this->exactly(2))
     ...
     * @return void
     */
    public function nestedSaveMany(CakeEvent $event): void
    {
        /** @var Model $model */
        $model = $event->subject;
     ...
     * @return void
     */
    public function nestedSaveAssociated(CakeEvent $event): void
    {
        /** @var Model $model */
        $model = $event->subject;
