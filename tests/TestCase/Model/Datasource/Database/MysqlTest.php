...
class MysqlTest extends CakeTestCase
{
    ...
    public function testIndexDetection()
    {
        ...
        $result = $this->Dbo->index('with_a_key');
        $this->Dbo->rawQuery('DROP TABLE ' . $name);
        $this->assertEquals($expected, $result);
        ...
        $result = $this->Dbo->index('with_two_keys');
        $this->Dbo->rawQuery('DROP TABLE ' . $name);
        $this->assertEquals($expected, $result);
        ...
        $result = $this->Dbo->index('with_compound_keys');
        $this->Dbo->rawQuery('DROP TABLE ' . $name);
        $this->assertEquals($expected, $result);
        ...
        $result = $this->Dbo->index('with_multiple_compound_keys');
        $this->Dbo->rawQuery('DROP TABLE ' . $name);
        $this->assertEquals($expected, $result);
        ...
        $result = $this->Dbo->index('with_fulltext');
        $this->Dbo->rawQuery('DROP TABLE ' . $name);
        $this->assertEquals($expected, $result);
        ...
        $result = $this->Dbo->index('with_text_index');
        $this->Dbo->rawQuery('DROP TABLE ' . $name);
        $this->assertEquals($expected, $result);
        ...
        $result = $this->Dbo->index('with_compound_text_index');
        $this->Dbo->rawQuery('DROP TABLE ' . $name);
        $this->assertEquals($expected, $result);
    }
    ...
    public function testIndexOnMySQL4Output()
    {
        ...
        $fetchReturns = array_map(fn($data) => (object)$data, $columnData);
        $fetchReturns[] = false;
    ...
    public function testBlobSaving()
    {
        ...
        $this->Dbo->cacheSources = false;
        $data = file_get_contents(CORE_TESTS . DS . 'test_app' . DS . 'webroot' . DS . 'img' . DS . 'cake.power.gif');
...
SQL;
    ...
    {
        $serverType = $this->Dbo->getServerType();
        $this->assertContains($serverType, ['MySQL', 'MariaDB', 'Aurora MySQL']);
    ...
    public function testGenerateAssociationQuerySelfJoin()
    {
        ...
        foreach ($this->Model->Category2->associations() as $type) {
            foreach ($this->Model->Category2->{$type} as $assoc => $assocData) {
                ...
                if (
                    $this->Model->Category2->alias === $linkModel->alias &&
    ...
    {
        $Article = ClassRegistry::init('Article', 'Comment');
        $Article->virtualFields = [
